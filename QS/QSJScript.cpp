...
HRESULT CQSJScript::FinalConstruct()
{
	...
	CComVariant varObj;
	CHECKHR(Parse(CComBSTR(L"{\"a\": 1}"), &varObj));
	CHECKHR(SetObjKey(varObj, CComVariant(L"b"), CComVariant(3.1415)));
	CHECKHR(SetObjKey(varObj, CComVariant(L"c"), CComVariant(L"Hello")));
	CComBSTR bstrJSON;
	CHECKHR(Stringify(varObj, CComVariant(), CComVariant(2), &bstrJSON));
	OutputDebugString(L"JSON: ");
	OutputDebugString((BSTR) bstrJSON);
	OutputDebugString(L"\r\n");
	return S_OK;
}
...
STDMETHODIMP CQSJScript::Parse(BSTR bstrJSON, VARIANT* pvarJsObject)
{
	HRESULT hr = S_OK;
	if (!pvarJsObject) return E_INVALIDARG;
	VariantInit(pvarJsObject);
	if (!m_spIQSScriptSite) return E_POINTER;
	CHECKHR(m_spIQSScriptSite->InvokeMethod(CComBSTR(L"jsParse"), CComVariant(bstrJSON), CComVariant(), CComVariant(), pvarJsObject));
	return S_OK;
}
...
STDMETHODIMP CQSJScript::SetObjKey(VARIANT varJsObject, VARIANT varKey, VARIANT varValue)
{
	HRESULT hr = S_OK;
	if (!m_spIQSScriptSite) return E_POINTER;
	CComVariant varResult;
	CHECKHR(m_spIQSScriptSite->InvokeMethod(CComBSTR(L"jsSetObjKey"), varJsObject, varKey, varValue, &varResult));
	return S_OK;
}
...
STDMETHODIMP CQSJScript::GetObjKey(VARIANT varJsObject, VARIANT varKey, VARIANT* pvarValue)
{
	HRESULT hr = S_OK;
	if (!pvarValue) return E_INVALIDARG;
	VariantInit(pvarValue);
	if (!m_spIQSScriptSite) return E_POINTER;
	CHECKHR(m_spIQSScriptSite->InvokeMethod(CComBSTR(L"jsGetObjKey"), varJsObject, varKey, CComVariant(), pvarValue));
	return S_OK;
}
...
STDMETHODIMP CQSJScript::Stringify(VARIANT varJsObject, VARIANT varReplacer, VARIANT varSpace, BSTR* pbstrJSON)
{
	HRESULT hr = S_OK;
	if (!pbstrJSON) return E_INVALIDARG;
	*pbstrJSON = NULL;
	if (!m_spIQSScriptSite) return E_POINTER;
	CComVariant varResult;
	CHECKHR(m_spIQSScriptSite->InvokeMethod(CComBSTR(L"jsStringify"), varJsObject, varReplacer, varSpace, &varResult));
	CComBSTR bstrJSON;
	CHECKHR(VariantToBSTR(varResult, &bstrJSON));
	*pbstrJSON = bstrJSON.Detach();
	return S_OK;
}
